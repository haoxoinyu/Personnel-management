...
@Component({
  ...
  styleUrls: ['./incubator.component.sass']
})
export class IncubatorComponent implements OnInit {
  ...
  loadByPage(page = 1): void {
    ...
    this.httpClient.post<Page<Incubator>>('/api/incubator', this.searchParameters, {params: httpParams})
      .subscribe(pageData => {
          // 在请求数据之后设置当前页
          this.page = page;
  ...
  onDelete(index: number, id: number): void {
    this.sweetAlertService.showWarning('', '', 'warning')
      .then(isConfirmed => {
        if (isConfirmed) {
          this.httpClient.delete(`/api/incubator/delete/${id}`)
            .subscribe(() => {
                console.log('删除成功');
  ...
  onSubmit(form: NgForm, page = 1) {
    ...
    if (form.valid) {
      ...
      const httpParams = new HttpParams().append('page', this.page.toString())
        .append('size', this.size.toString());
      this.httpClient.post<Page<Incubator>>('/api/incubator', this.searchParameters, {params: httpParams}).subscribe(
        pageData => {
          // 在请求数据之后设置当前页
          this.page = page;
          console.log('重新设置pageData');
